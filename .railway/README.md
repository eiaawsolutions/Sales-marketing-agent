...
> **This repo:** Railway does not read this file on deploy. `railway config apply`
> stores the settings on the service. It replaced `railway.toml` (Config as Code
> stopped being read on 2026-12-01). Always `railway config plan` before `apply`.
> The pulled `env: { NAME: preserve() }` entries keep existing variable values —
> never delete them, or apply will remove those variables from production.
>
> **Windows:** `railway/iac` checks the CLI version by running `$env:_`, which Git
> Bash sets to a path Node can't execute and npm's `railway` shim can't satisfy.
> Run plan/apply from PowerShell with
