...
export function checkPlanLimit(req, resource) {
  ...
  switch (resource) {
    ...
    case 'contact_reveal': {
      const count = db.prepare(
        "SELECT COUNT(*) as c FROM activities WHERE user_id = ? AND (type = 'contact_reveal' OR description LIKE 'Contact revealed:%') AND created_at >= datetime('now', 'start of month')"
      ).get(userId);
